	...
	class GUIManager
	{
		...
		*/
		...
		function GUIManager($db_name, $table_name )
		{
			...
			if( !is_file( $lst_file ) )	{ throw new FileIOException(  'DB定義ファイルが開けません。->'. $lst_file  ); }
			
			$fps[0] = fopen ($lst_file, 'r');
            if($fps[0] ==  FALSE ){ throw new FileIOException('DB定義ファイルのオープンに失敗しました。->'. $db_name); }
		...
		{
			if(  !is_file( $html )  )	{ throw new FileIOException( 'HTMLファイルが開けません。->'. $html ); }
        ...
        private function getDefState( $draw ){
        	return Array( 
        		...
        		,'if'=>false		//ifbeginの分岐で非表示とされている時にtrue
