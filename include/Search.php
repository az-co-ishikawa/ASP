...
class Search
{
	 ...
	 */
	function Search( &$gm = null, $type = null )	{ $this->gm = &$gm; $this->type = $type;}
	...
	function setParamertorSet(&$args){
		...
		for($i=0; $i<count($db->colName); $i++)
		{
			...
			if(  isset( $args[ $column_name. '_PAL'] )  )
			{
				...
				{
					// 値が配列の場合はエラー
					throw new InvalidArgumentException('Search param error -> '. $args[ $column_name ]. '_PAL[] is not array.');
				}
				...
				// 条件式に値があるかどうか
				throw new InvalidArgumentException('Search param error -> '. $column_name. '_PAL[] is null.');
			}
		}
	...
	function searchTable( $db , $table , $name , $param , $data , $join_db = null ){
		...
		switch( $param[0] )
		{
			case 'alias'://古い記法  現在は非推奨。  新しいもに関してはsearchAliasを参照
				...
				if( is_array( $data ) )
				{// 値が配列の場合はエラー
					throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is alias. but '. $name .' is array.');
				}
				...
				if( $param[5] == 'between' ){
					...
					throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is alias+between. but between is alias.');
				}
				...
				if( count($param) > 6 ){
					...
					for( $k=6 ; $k < $cnt ; $k+=3 ){
						if(   is_array(  $this->value[$param[$k+1]]  )   ){
							// 値が配列の場合はエラー
							throw new InvalidArgumentException('Search param error -> '. $param[$k] . '_PAL[] is alias. but '. $param[$k] .' is array.');
						}
						if( $param[$k+2] == 'comp' )
			...
			case 'match':
				...
				switch( $param[1] )
				{
					case 'comp':
						...
						if(   is_array(  $data  )   )
						{// 値が配列の場合はエラー
							throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is match+comp. but '. $name .' is array.');
						}
					...
					case 'like':
						...
						if(   is_array(  $data  )   )
						{// 値が配列の場合はエラー
							throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is match+like. but '. $name .' is array.');
						}
						$table = $this->searchExecute( $db, $join_db, $table, $name, '=', '%'. ($data). '%' );
						break;
					case 'keyword':
	...
	function searchOR( &$db , $table , $name, $data, $join_db = null , $param = null ){
		...
		for($k=0; $k<count($data); $k++)	{
			...
			$ttable[]	 = $this->searchExecute( $db, $join_db, $table, $name, '=', $str );
		}
		$table	 = $ttable[0];
		for($k=1; $k<count($data); $k++)	{ $table		 = $db->orTable( $table, $ttable[$k] ); }
		return $table;
	}
	function searchIN( &$db , $table , $name, $data , $join_db = null ){
		...
		{
			// 値が配列じゃない場合はエラー
			throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is match+or. but '. $name .' is not array.');
		}
	...
	function searchAND( &$db , $table , $name, $data , $join_db = null ){
		...
		$ttable	 = array();
		for($k=0; $k<count($data); $k++)	{ $ttable[]	 = $this->searchExecute( $db, $join_db, $table, $name, '=', '%'.($data[$k]).'%' ); }
		$table	 = $ttable[0];
		for($k=1; $k<count($data); $k++)	{ $table		 = $db->andTable( $table, $ttable[$k] ); }
		return $table;
	}
	...
	function searchBetween( &$db , $table , $name, $dataA, $dataB , $join_db = null ){
		...
		switch( $this->gm->colType[ $name ] ){
			...
			case 'double':
				...
				break;
			default:
				throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is between. but '. $name .' is not number.');
		}
		return $table;
	}
	...
	function searchFile( &$db , $table , $name , $data , $file ){
		...
		{
			// グルーピングファイルが見つからない場合
			throw new FileIOException( 'Search param error -> '. $name. '_PAL[] is group. but group file not found : ./group/'. $file );
		}
		if(   is_array(  $data  )   )
		{
			// 値が配列の場合はエラー
			throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is group. but '. $name .' is array.');
		}
	...
	function searchMultipleTable( &$db , $table , $name , $data , $param, $type , $join_db = null ){
		...
		for($k=0;$k<$cnt;$k++){
			...
			{
				// テーブル名が存在しない場合エラー
				throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is group. but table name not found.');
			}
	...
	function searchKeyword( &$db , $table , $name , $data , $param , $join_db = null ){
		...
		{
			// 値が配列の場合はエラー
			throw new InvalidArgumentException('Search param error -> '. $name. '_PAL[] is match+keyword. but '. $name .' is array.');
		}
