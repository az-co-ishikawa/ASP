<?php
	...
	class Command extends command_base
	{
		...
		function now( &$gm, $rec, $args ){
			...
			$add	 = isset($args[1])?(int)$args[1]:0;
			$format	 = isset($args[2])?$args[2]:$gm->timeFormat;
			...
			switch( $kind ){
				...
				case 'format':
				case 'f':
					$this->addBuffer( date( $format ) );
					break;
				default:
					$this->addBuffer( $this->addBuffer(SystemUtil::mb_date( $gm->timeFormat ) ) );
			}
		}
        ...
        function tableMultipleForm( &$gm , $rec , $args ){
            ...
            $this->addBuffer( $gm->getCCResult( $rec, '<!--# form multiple '.$args[0].' '.$check.' '.$value.' '.$index.' '.$args[4].$option.' #-->' ) );
        }
        ...
        function tableCheckForm( &$gm , $rec , $args ){
            ...
            for($i=0;$i<$row;$i++){
                $rec = $db->getRecord( $table , $i );
                if (($i + 1) % 7 == 0) {
                    $index[] = '&nbsp;'.SystemUtil::systemArrayEscape($db->getData( $rec , $args[2] ) ) . '<br>';
                }
                else {
                    $index[] = '&nbsp;'.SystemUtil::systemArrayEscape($db->getData( $rec , $args[2] ) );
                }
        ...
        */
        ...
        global $loginUserRank;
            ...
            if( !strlen( $HTML ) ){
                throw new FileIOException('dos not template');
            }
