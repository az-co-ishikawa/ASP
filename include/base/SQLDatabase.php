...
class SQLDatabaseBase implements DatabaseBase
{
	 ...
	 */
	function SQLDatabaseBase($dbName, $tableName, $colName, $colType, $colSize ){
	}
	...
	function getRecord($table, $index){
		...
		if( !$result ){
			throw new RuntimeException("getRecord() : SQL MESSAGE ERROR. \n");
		}
	...
	function getFirstRecord($table){
		...
		if( !$result ){
			exit("getFirstRecord() : SQL MESSAGE ERROR. \n");
		}
	...
	function getDataList($table, $name ){
		...
		if( !$result ){
			throw new RuntimeException("getDataList() : SQL MESSAGE ERROR. \n");
		}
	...
	function deleteTable($table){
		...
		if( !$result ){
			throw new RuntimeException("deleteTable() : SQL MESSAGE ERROR. \n");
		}
		$this->cashReset();
		return;
	}
	...
	/**
	 * whereによって選択されるテーブルの行を完全に削除します。
	 * @param $table テーブルデータ
	 * @return 行数
	 */
	function realDeleteTable($table){
...
//			if(  $_FILES[ $this->colName[$i] ]['name'] != "" || $_POST[ $this->colName[$i] . '_filetmp' ] != "" ){
	...
	function getRow($table){
		...
		if( !$result ){
			if( !$this->_DEBUG ){ d( "getRow() : ". $sql. "<br/>\n", 'sql'); }
			throw new RuntimeException("getRow() : SQL MESSAGE ERROR. \n");
		}
			
		$rows = $this->sql_fetch_assoc($result,0);
		$row = $rows['cnt'];
		if( $row == -1 ){
			throw new RuntimeException("getRow() : GET ROW ERROR ( RETURN -1 ).\n");
		}
		$this->row = $row;
	...
	function existsRow($tbl){
		...
		if( ! is_null( $this->rec_cash )){
			...
			if( $this->_DEBUG ){ d( "existsRow() : ". $table->getString(). "<br/>\n", 'sql' ); }

			$result	 = $this->sql_query( $table->getString() );
		}

		if( !$result ){
			exit("existsRow() : SQL MESSAGE ERROR. \n");
		}
	...
	function joinTableLike( &$tbl, $b_name, $n_name, $b_col, $n_col, $n_tbl_name = null ){
		...
		return $this->joinTableSQL( $tbl, $b_name, $n_name, $_n_tbl_name.".".$n_col." like '%' || ".$_b_name.".".$b_col."|| '%' ", $n_tbl_name );
	}
	/**
	...
	function getSum( $name, $table = null){
		...
		if( !$result ){
			if( !$this->_DEBUG ){ d( "getSum() : select sum($name) as sum from ". strtolower($this->tableName).$table->getWhere(). "<br/>\n", 'sql'); }
			throw new RuntimeException("getSum() : SQL MESSAGE ERROR. \n");
		}
	...
	function sql_query($sql){
		throw new RuntimeException('SQLDatabase::'.__FUNCTION__.'  Unmounting');
	}
	...
	function sql_fetch_all($result){
		throw new RuntimeException('SQLDatabase::'.__FUNCTION__.'  Unmsounting');
	}
...
class TableBase{
	...
	function TableBase($from){$this->baseName = $from;}
	...
	function getRowString(){
		...
		if( $cTable->select == '*' && is_null( $cTable->limit ) ){
