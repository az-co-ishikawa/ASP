<?php
	...
	class ccProc
	{
		...
		{
			$ret			 = "";
			if(  !isset( $gm )  )	{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via value )' ); }
			if(   isset(  $_GET[ $cc[1] ]  )   ){
		...
		{
			$ret			 = "";
			if(  !isset( $gm )  )	{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via valueReplace )' ); }
			$db		 = $gm->getCachedDB();
		...
		{
			$ret			 = "";
			if(  !isset( $gm )  )	{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via valueValueReplace )' ); }
			$data	 = $cc[1];
			if( is_bool($data) )
			{
				if( $data )	{ $data	 = 'TRUE'; }
        ...
        {
			$ret			 = "";
			if( !isset($gm) )						{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via arrayReplace )' ); }
			$db		 = $gm->getCachedDB();
		...
		{
			$ret			 = "";
			if( !isset($gm) )						{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via alias )' ); }
		...
		{
			$ret			 = "";
			if( !isset($gm) )						{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via valueAlias )' ); }
		if( !isset( $gm->aliasDB[ $cc[1] ] ) )	{ $gm->addAlias( $cc[1] ); }
		...
		{
			$ret			 = "";
			if( !isset($gm) )						{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via arrayAlias )' ); }
			if( !isset( $gm->aliasDB[ $cc[1] ] ) )	{ $gm->addAlias( $cc[1] ); }
		...
		{
			$ret			 = "";
			if( !isset($gm) )						{ throw new RuntimeException( 'CommandComment Null Pointer Error -> $gm ( via arrayValueAlias )' ); }
			if( !isset( $gm->aliasDB[ $cc[1] ] ) )	{ $gm->addAlias( $cc[1] ); }
		...
		function object(&$gm, $rec, $cc)
		{
			...
			switch( $cc[1] )
			{
				case 'image':
					...
					else												{ $ret	 .= '<img'."\t".'src="'. $gm->db->getData($rec, $cc[2]). '"'."\t".'border="0"/>'; }
					break;
		...
		function form(&$gm, $rec, $cc)
		{
			...
			if(   isset(  $_GET[ $cc[2] ]  )   )
			{
				if( is_array( $_GET[ $cc[ 2 ] ] ) )
				{
					...
					foreach( $_GET[ $cc[ 2 ] ] as $element )
					{
						if( false !== strpos( $element , '%u' ) )
							{ $array[] = escuni2sjis( $element ); }
						else
							{ $array[] = $element; }
					}
			...
			switch( $cc[1] )
			{
				case 'text':
					...
					if(   isset(  $_POST[ $cc[2] ]  )   )	{ $option	 .= ' value="'. htmlspecialchars($_POST[ $cc[2] ]) .'"';	}
					else									{ $option	 .= ' value="'. htmlspecialchars($cc[5]) .'"'; }
				...
				case 'textarea':
					...
					$ret	 .= '<textarea name="'. $cc[2] .'" cols="'. $cc[3] .'" rows="'. $cc[4] .'" '. $option .'>'. htmlspecialchars ( $value, ENT_QUOTES ) .'</textarea>'. "\n";
					break;
				...
				case 'option':
					...
					if(   isset(  $_POST[ $cc[2] ]  )   )
					{
						for($i=0; $i<count($value); $i++)
						{
							if(  $value[$i] == $_POST[ $cc[2] ]  )	{ $ret	 .= '<option value="'. $value[$i] .'" selected="true" >'. $index[$i] .'</option>'. "\n"; }
					...
					else
					{
						for($i=0; $i<count($value); $i++)
						{
							if(  $value[$i] == $cc[3]  )	{ $ret	 .= '<option value="'. $value[$i] .'" selected="true" >'. $index[$i] .'</option>'. "\n"; }
							else							{ $ret	 .= '<option value="'. $value[$i] .'">'. $index[$i] .'</option>'. "\n"; }
						}
					}
					$ret	 .= '</select>'. "\n";
					...
					if( isset(  $_POST[ $cc[2] ]  ) )	{
						if( is_array( $_POST[ $cc[2] ] ) ){
							foreach( $_POST[ $cc[2] ] as $val ){
								$ret		 .= '<input name="'. $cc[2] .'[]" type="hidden" value="'. htmlspecialchars($val) .'" '. $option .'/>'. "\n";
							}
						}
						else{
							$ret		 .= '<input name="'. $cc[2] .'" type="hidden" value="'. htmlspecialchars(($_POST[ $cc[2] ])) .'" '. $option .'/>'. "\n";
						} 
					}
					else								{ $ret		 .= '<input name="'. $cc[2] .'" type="hidden" value="'. htmlspecialchars($cc[3]) .'" '. $option .'/>'. "\n"; }
					break;
			}
		...
		{
			$ret			 = "";
			if(  is_null( $gm->variable[ $cc[1] ] )  )	{ throw new InvalidCCArgumentException( 'CommandComment Null Pointer Error -> variable : '. $cc[1] ); }
		...
		function get(&$gm, $rec, $cc)
		{
			...
			{
				if(  !isset( $cc[2] )  )	{ throw new InvalidCCArgumentException( 'CommandComment Null Pointer Error -> get array index' ); }
				$ret	 .= $_GET[ $cc[1] ][ $cc[2] ];
			}
			else	{ $ret	 .= $_GET[ $cc[1] ]; }
		...
		function post(&$gm, $rec, $cc)
		{
			...
			{
				if(  !isset( $cc[2] )  )	{ throw new InvalidCCArgumentException( 'CommandComment Null Pointer Error -> post array index' ); }
				$ret	 .= $_POST[ $cc[1] ][ $cc[2] ];
			}
			else	{ $ret	 .= $_POST[ $cc[1] ]; }
		...
		function request(&$gm, $rec, $cc)
		{
			...
			{
				if(  !isset( $cc[2] )  )	{ throw new InvalidCCArgumentException( 'CommandComment Null Pointer Error -> request array index' ); }
